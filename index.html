...
#notif{position:fixed;bottom:1rem;right:1rem;z-index:9999;display:flex;flex-direction:column;gap:6px;pointer-events:none;max-height:45vh;overflow:hidden;}
...
@media(max-height:620px){.land-center{justify-content:flex-start;padding-top:1rem;padding-bottom:1rem;}.land-ball{font-size:2.5rem;}.land-divider{margin-bottom:1rem;}.land-sub{margin-bottom:1rem;}.land-feat{margin-bottom:1rem;}.land-btn-main{padding:.75rem 1.5rem;}}

/* ===== LANDING SCREEN ===== */
#landing{position:fixed;inset:0;z-index:9998;overflow-y:auto;overflow-x:hidden;background:#050a0e;}
  ...
  <div class="card">
    ...
    <div class="g2">
      ...
      <div>
        ...
        <hr class="div">
        <div class="ct" style="font-size:.88rem;">คู่แข่งตัวจริง</div>
        <div id="mp-opponent-xi" class="tm">เริ่มแข่งเพื่อดูรายชื่อนักเตะ AI</div>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="card">
      ...
      <div class="fb gap mb">
        ...
        <select id="s-sort" style="width:110px;" onchange="renderMarket()">
          ...
          <option value="affordable">ซื้อไหว</option>
        </select>
      </div>
      <div id="market-list"></div>
      <hr class="div">
      <div class="ct">🌐 รายการที่เราตั้งขาย</div>
      <div class="tm mbm" style="font-size:.8rem;">ผู้ซื้อจะเห็นราคาที่เราตั้งไว้ แล้วเสนอราคากลับมาให้กดรับ/ปฏิเสธ</div>
      <div id="market-online-listings"></div>
    </div>
  </div>
  ...
  <div class="card">
    <div class="fbtw mb">
      ...
      <div class="fb gap">
        ...
        <button class="btn bgr bsm" onclick="scoutYouth()">🔍 สอดแนม (50K)</button>
...
<div id="pg-relations" class="pg">
  ...
  <div class="card">
    ...
    <button class="btn bgr bsm" style="margin-top:.5rem;" onclick="spawnWonderkid()">🌟 ค้นหา Wonderkid (100K)</button>
  </div>
</div>
...
const MARKET_POOL_SIZE=180;
const MARKET_POOL_TIERS=['bronze','silver'];
...
function startMatch(){
  ...
  const oppOVR=G.oppXI.length?calcAIClubStrength(oppClub,G.oppXI):(G.leagueTable.find(t=>t.name===G.oppName)?.rating||getLeagueClubRating(G.oppName));
  const style=document.getElementById('t-style')?.value||'balanced';
...
function renderMatchDashboard(myStr=null,oppStr=null,finalResult=''){
  ...
  const home=slots.map((sl,i)=>{
    ...
    return `<div class="mp-dot mp-home" style="left:${sl.x}%;top:${sl.y}%;"><div class="mp-ball" style="background:${color};">${sl.p}</div><div class="mp-name">${name}</div><div class="mp-rating">★${Number(rating).toFixed(1)}</div></div>`;
  }).join('');
  ...
  const away=awayEntries.map(x=>{
    const name=(x.p?.name||G.oppName||'Opponent').split(' ').slice(-1)[0];
    return `<div class="mp-dot mp-away" style="left:${x.slot.x}%;top:${x.slot.y}%;opacity:.78;"><div class="mp-ball">${x.slot.p}</div><div class="mp-name">${name}</div>${x.p?`<div class="mp-rating">OVR ${x.p.ovr}</div>`:''}</div>`;
  }).join('');
...
function buyLegend(name){
  ...
  G.squad.push({...player,acquisition:'legend',isInitialSquad:false});updateHUD();notify(`⭐ ${p.name} เข้าร่วมทีม!`,'green');
}
...
function recordAISimulatedFixture(club,goals){
  ...
  xi.forEach(x=>x.p.apps=(x.p.apps||0)+1);
...
function addPackPlayers(){
  ...
  add.forEach(p=>G.squad.push({...p,id:uid(),acquisition:'pack',isInitialSquad:false}));
...
function scoutYouth(){
  ...
  const p=genPlayer({age:rnd(15,20),base:rnd(45,62),potential:rnd(potMin+scoutBonus,potMax+scoutBonus),real:false});
